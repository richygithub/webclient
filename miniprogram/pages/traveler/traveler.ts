...
Page({
  ...
  submitForm() {
    ...
    const { travelers    } = this.data;
    wx.request<CourseInfo>({
      url: `${getApp().globalData.apiBase}/user/addTraveler`,
      method: 'POST',
      data: { travelers, "token": getApp().globalData.token },
      success: (res) => {
        console.log('请求成功:', res.data); // res.data为服务器响应内容
        getApp().globalData.baseInfo.travelers = res.data
        wx.navigateBack();
        this.setData({
